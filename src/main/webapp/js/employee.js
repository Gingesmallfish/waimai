...
function queryEmployee() {
    ...
    queryParams.employeeName = $.trim($('input[textboxname=employeeName]').textbox('getValue'));
...
function addEmployee(employeeid) {
    ...
    $('#employee-dialog').dialog({
        ...
        /**
         * 设置对话框窗口顶部工具栏，可用值有：
         * 1) 一个数组，每一个工具栏中的工具属性都和 linkbutton 相同。
         * 2) 一个选择器指定工具栏。
    ...
    $('input[textboxname=name]').textbox().next('span').find('input').focus();
}

function editEmployee(row) {
    if (row && row['id'] != undefined) {
        employee_id = row['id'];
        $('input[textboxname=name]').textbox('setValue', row['name']);
        $('#sex').combobox('setValue',row['sex']);
        $('input[textboxname=id_number]').textbox('setValue', row['id_number']);
        $('input[textboxname=phone]').textbox('setValue', row['phone']);
        $('#employee-dialog').dialog({
            closed: false,
            modal: true,
            collapsible:true,
...
function saveEmployee() {
    if ($.trim($('input[textboxname=name]').textbox('getValue')) == "") {
        $.messager.show({
            title: '系统提示',
            msg: "请填写员工姓名"
        });
    ...
    if ($.trim($('input[textboxname=id_number]').textbox('getValue')) == "") {
        $.messager.show({
            title: '系统提示',
            msg: "请填写身份证号"
        });
        $('input[textboxname=id_number]').textbox().next('span').find('input').focus();
        return;
    }
    if ($.trim($('input[textboxname=phone]').textbox('getValue')) == "") {
        $.messager.show({
            title: '系统提示',
            msg: "请填写手机号"
        });
        $('input[textboxname=phone]').textbox().next('span').find('input').focus();
        return;
    }
        ...
        success: function(result, status, xhr, $form) {
            ...
            if (result.code) {
                if (employee_id == '') {
                    ...
                    $('#sex').combobox('setValue','-1');
                    $('input[textboxname=id_number]').textbox('setValue', '');
